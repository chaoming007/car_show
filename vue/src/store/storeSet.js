...
export default{
    state:{
        ...
        selectCarIndex:{id:0,index:0},                  //对比滑动选车index索引
        stySetVal:true,
        isLoad:true                                    //是否加载完成
        //对比按钮状态
    },
    mutations:{
        ...
        currentVideo(state,dat){                //当前播放视频
            for(let i=0;i<state.carDat.length;i++){
                if(dat==state.carDat[i].id){
                    ...
                    state.currentVideoDat.index=state.selectCarIndex.index
                    //state.currentVideoDat.id=state.selectCarIndex.id
                    state.contrastCarArr[0].dat=state.carDat[i]
                    // if(state.contrastCarArr[0].dat){
                    //     state.contrastCarArr[0].dat.index=state.selectCarIndex.index
                    // }
                    break
                }
            }
            ...
            //state.currentVideoDat=state.carDat[0]
                //state.currentVideoDat.index=state.selectCarIndex.index
                // state.contrastCarArr[0].dat=state.carDat[0]
                // state.contrastCarArr[0].dat.index=12
        ...
        changeSelectCarIndex(state,dat){       //更新对比选车索引index
            state.selectCarIndex.id=dat.id
            state.selectCarIndex.index=dat.index
        },
        deldbFun(state,dat){
            state.contrastCarArr[1].dat=dat
        }
    ...
    actions:{
        ...
        changeSelectCarIndex({commit},dat){
            commit("changeSelectCarIndex",dat)
        },
        deldbFun({commit},dat){
            commit("deldbFun",dat)
        }
    ...
    getters:{
        ...
        selectCarIndex(state){
            return state.selectCarIndex
        }
